...
$process_parameters = array_merge($process_parameters,$varsvalues);
			 ...
			 $process_parameters["checksum"] = $this->sign($process_parameters, $qp_apikey);

    
       reset($process_parameters);
        while (list ($key, $value) = each($process_parameters)) {
            $process_fields .= zen_draw_hidden_field($key, $value) . "\n";
        }
...
$order->info['cc_expires'] = ($qp_expire ? $qp_expire : 'N/A');
$order->info['order_status'] = $order_status_id;
  //clean up preparing order, let ZenCart create additional order data

                //    $db->Execute('delete from ' . TABLE_ORDERS_TOTAL . ' where orders_id = "' . (int) $order_id . '"');
                $db->Execute('delete from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int) $order_id . '" and orders_status_id = "'. MODULE_PAYMENT_QUICKPAY_ADVANCED_PREPARE_ORDER_STATUS_ID.'" ');
                        $db->Execute('delete from ' . TABLE_ORDERS_PRODUCTS . ' where orders_id = "' . (int) $order_id . '"');
                        $db->Execute('delete from ' . TABLE_ORDERS_PRODUCTS_ATTRIBUTES . ' where orders_id = "' . (int) $order_id . '"');
                        $db->Execute('delete from ' . TABLE_ORDERS_PRODUCTS_DOWNLOAD . ' where orders_id = "' . (int) $order_id . '"');
    ...
    $comment_status = "Transaction: ".$st[0]["id"] . $qp_pending.' (' . $qp_cardtype . ' ' . $currencies->format($qp_amount / 100, false, $qp_currency) . ') '. $qp_status_msg;
...
//	}else{
            $sql_data_array = array('orders_id' => $qp_order_id,
                ...
                'date_added' => 'now()',
                'customer_notified' => '0',
                'comments' => 'QuickPay Payment Verification successfull [ ' . $comment_status . ']');
            zen_db_perform(TABLE_ORDERS_STATUS_HISTORY, $sql_data_array,'update', "orders_id = '" . $qp_order_id . "' and orders_status_id ='".MODULE_PAYMENT_QUICKPAY_ADVANCED_PREPARE_ORDER_STATUS_ID."' ");
